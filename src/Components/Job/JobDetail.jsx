...
function JobDetail
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await axios.get(`https://internareaback.onrender.com/api/job/${id}`)
  ...
  const submitApplication = async () => {
    ...
    if (text.value === "  ") {
      ...
      await axios.post("https://internareaback.onrender.com/api/application", bodyJson).then((res) => {
      }).catch((err) => {
        alert("error happend")
      })
      alert("Done")
      ...
      <div className="details-app">
        ...
        <div className="m-14 shadow-sm rounded-md border">
          ...
          <div className="flex tedxt-sm justify-between">
            <p className='mt-3 text-slate-600'> <i class="bi bi-play-circle-fill"></i>   Start Date  <br />  {data.StartDate}</p>
            <p className='mt-3 text-slate-600' > <i class="bi bi-calendar-check-fill"></i>  Experience  <br />{data.Experience}</p>
            <p className='mt-3 text-slate-600'>  <i class="bi bi-cash"></i>   Salary <br /> {data.CTC}</p>
          </div>
          <div className="flex">
          ...
          <div className='flex justify-center'>
            <button class="apply" onClick={show}>Apply</button>
          </div>
        </div>
      </div>
      {isDivVisible && (
        <>
          <div className="application-page">
            <div className="bg">
              <button className='close2' onClick={hide} ><i className='bi-bi-x'>X</i></button>
              <p>Applying for Company {data.company}</p>
            ...
            <div>
              <label>
                ...
                Other 
                <span className='text-slate-500'>(Please specify your availability)  </span>
              </label>
            </div>
